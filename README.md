...
- **随机分支节点**:先按条件过滤出满足的分支(支持并/或),再**随机走其中一条**;无条件分支永远参与随机;全不满足走默认出口
...
- **图内快捷键**:`Ctrl/Cmd + S` 保存;`Ctrl/Cmd + C` 复制选中节点;`Ctrl/Cmd + V` 粘贴到鼠标位置(可连续粘贴,互不影响;开始节点不可复制)
...
| 随机分支节点 | 在所有满足条件的分支中随机走一条 | 每条分支一个端口 + 默认 |
...
- 每条分支的条件用法与状态分支一致(支持并/或组合),但**不是**自上而下取第一条,而是所有满足者等概率随机。
- **无条件分支永远参与随机池**(与状态分支"空条件永不命中"不同)——纯随机场景直接建几条不挂条件的分支即可。
- 所有分支都有条件且全不满足时走末尾“**默认**”端口,连接兜底路径。
- 随机源是全局 `System.Random` 单例;需要复现(测试/回放)时在播放前调用 `RandomBranchNode.ResetSeed(seed)`。
